...
export const customerValidation = z.object({
  // Required fields
  name: z.string().min(1, { message: "Name is required" }).max(30, {message: "Name must be less than 30 characters"}),
  contactNumber: z.string().min(8, { message: "Contact number is required" }).max(15, "Phone Number be less than 15 charcaters").trim(),
  ...
  // Enum fields
  installedModel: z.enum([
    'NMP-5', 'NMP-7', 'NMP-9', 'NMP-11', 
    'UCE-9', 'UCE-11', 'UCE-13', 
    'Hbride-H2', 'H-rich'
  ]).optional(),
  amcRenewed: z.enum(["YES", "NO"]).optional(),
