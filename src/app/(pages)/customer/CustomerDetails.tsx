...
const CustomerDetails = ({ customerId }: { customerId: string }) => {
  ...
  if (error) return <div className="text-red-600 p-4">Error loading customer data</div>;
    ...
    <div className="p-4 space-y-6">
      ...
      <section className="border p-4 rounded-lg shadow-sm bg-white">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm text-gray-700">
          ...
          <p><strong>Serial No:</strong> {customer.serialNumber}</p>
      ...
      {/* ========== 🔧 UPCOMING SERVICES ========== */}
      <section className="border p-4 rounded-lg shadow-sm bg-white">
        <h2 className="text-lg font-semibold mb-2 text-gray-800">📅 Upcoming Services</h2>
            ...
            {customer.upcomingServices.map((service: any) => (
              <li key={service._id} className="py-2">
                <div className="flex justify-between">
                  ...
                  <span>{format(new Date(service.serviceDate), "dd MMM yyyy")}</span>
                </div>
      ...
      {/* ========== 🛠️ SERVICE HISTORY ========== */}
      <section className="border p-4 rounded-lg shadow-sm bg-white">
        <h2 className="text-lg font-semibold mb-2 text-gray-800">🕓 Service History</h2>
            ...
            {customer.serviceHistory.map((history: any, index: number) => (
              <li key={index} className="py-2">
                <div className="flex justify-between">
                  ...
                  <span>{format(new Date(history.serviceDate), "dd MMM yyyy")}</span>
                </div>
                <div className="text-gray-600 mt-1">
